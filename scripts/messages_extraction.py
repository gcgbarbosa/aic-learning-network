...
def get_messages_from_chtbot(interaction_id: str) -> dict[str, list]:
    ...
    for r in messages:
        ...
        if chatbot:
            message_exchange[chatbot.name].append({"sender": "chatbot", "message": r.content, "timestamp": r.timestamp})  # type: ignore[attr-defined]
        else:
            for chatbot_name in message_exchange.keys():
                message_exchange[chatbot_name].append(
                    {"sender": "user", "message": r.content, "timestamp": r.timestamp}  # type: ignore[attr-defined]
                )
